...
function renderList(list) {
  ...
  if (list.length !== 0) {
    list.forEach((p) => {
      ...
      pokeItem.classList.add("list-inline-item");
      let list = document.createElement("btn");
      list.textContent = p.name;
      list.classList.add("btn");
      list.classList.add("btn-light");

      pokeItem.append(list);

      $("#pokemon-list").append(pokeItem);
    });
  } else {
    $("#pokemon-list").append("<p>No pokemon matches</p>");
  }

  $("btn").click((e) => {
    const currentPokemon = e.currentTarget.textContent;
    const url = "https://pokeapi.co/api/v2/pokemon/" + currentPokemon;

    let pokeImg = document.getElementById("pokemon-img");
    pokeImg.classList.add("in-transition");
    $(".poke-info")
      .delay("300")
      .fadeIn(() => {
        ...
        fetch(url, requestOptions)
          ...
          .then((result) => {
            ...
            pokeImg.src = imgUrl;
            pokeImg.alt = currentPokemon;

            console.log(pokemon);

            $("#poke-title").text(currentPokemon);
            $("#description").text("#" + pokemon.id);
            $("#link").show(() => {
              document.getElementById("link").href =
                "pages/fiche.html?n=" + currentPokemon;
            });
            $(".poke-info")
              .delay("300")
              .fadeIn("300", () => {
                pokeImg.classList.remove("in-transition");
              });
          })
          .catch((error) => {
            pokeImg.src = "ash_sad.jpg";
            pokeImg.alt = "Error";

            $("#poke-title").text(currentPokemon);
            $("#description").text(
              "There was an error, I could not find " + currentPokemon
            );
            $("#link").hide();
            $(".poke-info")
              .delay("300")
...
$("input").keyup((e) => {
  ...
  if (search !== "") {
    const list = pokemon.filter(
      (p) => search === p.name.substr(0, search.length)
    );
    renderList(list);
  } else {
    renderList(pokemon);
